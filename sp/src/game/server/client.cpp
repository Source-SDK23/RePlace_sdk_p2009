...
{
	ClassNamePrefix_t(const char* pszPrefix, bool bKeepPrefix, int nType) : m_pszPrefix(pszPrefix), m_bKeepPrefix(bKeepPrefix), m_nType(nType) {
	...
	size_t m_nLength;
	bool m_bKeepPrefix;
	// Type: 0 - Item, 1 - Weapon (Mainly used for give_weapon command)
	int m_nType;
};

static ClassNamePrefix_t s_pEntityPrefixes[] =
{
	ClassNamePrefix_t("item_", false, 0),
	ClassNamePrefix_t("weapon_", false, 1),
};
...
int GiveAutocomplete(const char* partial, char commands[COMMAND_COMPLETION_MAXITEMS][COMMAND_COMPLETION_ITEM_LENGTH])
{
	...
	for (int i = factoryDict.First(); i != factoryDict.InvalidIndex() && numMatches < COMMAND_COMPLETION_MAXITEMS; i = factoryDict.Next(i)) {
		...
		for (j = 0; j < ARRAYSIZE(s_pEntityPrefixes); j++) {
			...
			if (Q_strncmp(pszClassName, pPrefix->m_pszPrefix, pPrefix->m_nLength)) continue;
			break;
		}

		if (j == ARRAYSIZE(s_pEntityPrefixes)) continue;

		if (!pPrefix->m_bKeepPrefix) pszClassName += pPrefix->m_nLength;

		if (Q_strnicmp(pszClassName, partial, partialLength)) continue;
