...
private:
	float m_flFlyTime;
	CSoundParameters soundParms;
	const char* m_sLaunchSound;
	SoundFlags_t flags;
};
...
DEFINE_KEYFIELD(m_sLaunchSound, FIELD_SOUNDNAME, "launchsound"),
END_DATADESC()

ConVar sv_launchpad_airaccelerate("sv_launchpad_airaccelerate", "0.75", FCVAR_SERVER_CAN_EXECUTE, "Toggle's launch pads not allowing air movement until you land.");
...
void CTriggerCatapult::Touch(CBaseEntity* pOther)
{
	...
	// Launch Sound Code
	if (m_sLaunchSound) {
		/* CRecipientFilter filter1;
		filter1.AddRecipient(pPlayer);
		CBaseEntity::EmitSound(filter1, SOUND_FROM_LOCAL_PLAYER, m_sLaunchSound);
		CRecipientFilter filter2;
		filter2.AddAllPlayers();
		filter2.RemoveRecipient(pPlayer); */
		if (!GetParametersForSound(m_sLaunchSound, soundParms, NULL)) { // For raw sounds
			PrecacheSound(m_sLaunchSound);
		}
		else { // For Soundscripts
			PrecacheScriptSound(m_sLaunchSound);
		}
		UTIL_EmitAmbientSound(RequiredEdictIndex(), GetAbsOrigin(), m_sLaunchSound, 1, SNDLVL_NORM, flags, PITCH_NORM);
	}


	if (pPlayer) {
		pPlayer->m_fLaunchpadAirAccelerate = sv_launchpad_airaccelerate.GetFloat();
	}
}
#endif